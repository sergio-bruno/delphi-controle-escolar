...
type
  TFrm_GerenciaAluno = class(TForm)
    Panel1: TPanel;
    Panel2: TPanel;
    Panel3: TPanel;
    GroupBox1: TGroupBox;
...
var Frm_GerenciaAluno: TFrm_GerenciaAluno;
const wSelect1 = 'SELECT MAT.*,ALU.ALUNNOME,SER.SERITURN,SER.SERIDESC,TRN.TURNDESC,';
      wSelect2 = 'AGE.AGENNOME ,DIS.DISCDESC,ALU.ALUNINAT FROM TB_MATRICULA MAT ';
...
uses U_FiltroGerenciamentoAluno, u_modulo, u_funcoes, U_SelecionaAlunoMatricula,
     U_AlunosPorSerie, U_TbSerie, U_EditaGerenciamentoAluno, U_Principal;

{$R *.dfm}

procedure TFrm_GerenciaAluno.FormKeyPress(Sender: TObject; var Key: Char);
begin
     If (Key = #13) then
     begin
        key:= #0;
...
procedure TFrm_GerenciaAluno.Btn_AlunoSerieClick(Sender: TObject);
begin
...
procedure TFrm_GerenciaAluno.FormShow(Sender: TObject);
begin
     //afuncoes.AbreQuery(Frm_Modulo.IBQ_Matricula,wSelect1+wSelect2+wSelect3+
     //wSelect4+wSelect5+wSelect6+wSelect7+wOrdem);
end;

procedure TFrm_GerenciaAluno.FormClose(Sender: TObject;
...
procedure TFrm_GerenciaAluno.btn_matriculaClick(Sender: TObject);
begin
     with TFrm_SelecionaAlunoMatricula.Create(Self) do
     try
        caption := Frm_Principal.wNomeEmpresa;
        showmodal;
     Finally
...
procedure TFrm_GerenciaAluno.Btn_filtrarClick(Sender: TObject);
var wselect8 : string;
begin
...
procedure TFrm_GerenciaAluno.Btn_editarClick(Sender: TObject);
begin
     with TFrm_EditaGerenciamentoAluno.Create(Self) do
     try
        caption := Frm_Principal.wNomeEmpresa;

        if Frm_Modulo.IBQ_Matricula.IsEmpty then
             LimpaVariavel('','','','')
        ...
        [Frm_Modulo.IBQ_Matricula.fieldbyname('matr_ano').asstring,
        Frm_Modulo.IBQ_Matricula.fieldbyname('matrseri').asstring,
        Frm_Modulo.IBQ_Matricula.fieldbyname('matralun').asstring,
        Frm_Modulo.IBQ_Matricula.fieldbyname('matrdisc').asstring]) then
             AtribuiVariavel
        else LimpaVariavel('','','','');
        showmodal;
     Finally
...
procedure TFrm_GerenciaAluno.NvelEscolar1Click(Sender: TObject);
begin
     Frm_principal.NvelEscolar1Click(Sender);
end;

procedure TFrm_GerenciaAluno.Disciplina1Click(Sender: TObject);
begin
     Frm_principal.Disciplina1Click(Sender);
end;

procedure TFrm_GerenciaAluno.Srie1Click(Sender: TObject);
begin
...
procedure TFrm_GerenciaAluno.Docente1Click(Sender: TObject);
begin
     Frm_Principal.Clientes1Click(Sender);
end;

procedure TFrm_GerenciaAluno.Dicente1Click(Sender: TObject);
begin
     Frm_Principal.Fornecedor1Click(Sender);
end;

procedure TFrm_GerenciaAluno.RxDBGrid1GetCellParams(Sender: TObject;
  Field: TField; AFont: TFont; var Background: TColor; Highlight: Boolean);
begin
  if Frm_Modulo.IBQ_Matricula.fieldbyname('matrrsfn').asstring = 'R' then
  begin
       AFont.Color := clRed;
       Background  := clInfoBk;
       AFont.Style := AFont.Style + [fsBold];
  end
  else if Frm_Modulo.IBQ_Matricula.fieldbyname('matrrsfn').asstring = 'A' then
  begin
       AFont.Color := clBlue;
       Background  := clInfoBk;
       AFont.Style := AFont.Style + [fsBold];
...
procedure TFrm_GerenciaAluno.btn_sairClick(Sender: TObject);
begin
     close;
end;
