unit U_SelecionaAlunoMatriculaC;
...
type
  TFrm_SelecionaAlunoMatriculaC = class(TForm)
    Panel1: TPanel;
    DBGrid1: TDBGrid;
    Panel2: TPanel;
    GroupBox1: TGroupBox;
...
var Frm_SelecionaAlunoMatriculaC: TFrm_SelecionaAlunoMatriculaC;
...
procedure TFrm_SelecionaAlunoMatriculaC.FormKeyPress(Sender: TObject; var Key:
          Char);
begin
     If (Key = #13) then
     begin
...
procedure TFrm_SelecionaAlunoMatriculaC.FormShow(Sender: TObject);
begin
...
procedure TFrm_SelecionaAlunoMatriculaC.FormClose(Sender: TObject;
...
procedure TFrm_SelecionaAlunoMatriculaC.Rdg_OrdemClick(Sender: TObject);
begin
...
procedure TFrm_SelecionaAlunoMatriculaC.Button1Click(Sender: TObject);
var i, j: Integer;
    wzmes, wvenc : string;
begin
     ...
     begin
          ...
          begin
               ...
               begin
                    ...
                    try
                       ...
                       begin
                         close;
                         Sql.clear;
                         Sql.add('INSERT INTO tb_MatrConceito (MATR_ANO,MATRSERI,');
               ...
               // CADASTRANDO O CONCEITO
               afuncoes.AbreQuery(Frm_Modulo.IBQ_funcoes2,'Select * from tb_Conceito');
               ...
               begin
                    ...
                    [Edt_ano.text,Edt_serie.Text,DBGrid1.DataSource.DataSet.Fields[0].AsString,
                    Frm_Modulo.IBQ_funcoes2.fieldbyname('conccodg').asstring]) then
                    try
                       If Not(Frm_Modulo.IBT_Transaction.InTransaction) Then
                          Frm_Modulo.IBT_Transaction.StartTransaction;
                       with Frm_Modulo.IBQ_funcoes do
                       begin
                         close;
                         Sql.clear;
                         Sql.add('INSERT INTO tb_MatrTabConceito (MATR_ANO,MATRSERI,');
                         Sql.add('MATRALUN,MATRCONC)');
                         Sql.add('VALUES (:WMATR_ANO,:WMATRSERI,:WMATRALUN,');
                         Sql.add(':WMATRCONC)');
                         parambyname('WMATR_ANO').asstring := edt_ano.text;
                         parambyname('WMATRSERI').asstring := edt_serie.text;
                         parambyname('WMATRALUN').asstring := DBGrid1.DataSource.DataSet.Fields[0].AsString;
                         parambyname('WMATRCONC').asstring := Frm_Modulo.IBQ_funcoes2.fieldbyname('conccodg').asstring;
                         if not prepared then prepare;
                         execsql;
                       end;
                       Frm_Modulo.IBT_Transaction.CommitRetaining;
...
//------------------------------------------------------------------------------
Procedure TFrm_SelecionaAlunoMatriculaC.FiltraDisciplina(WCodigo: string);
//------------------------------------------------------------------------------
begin
...
procedure TFrm_SelecionaAlunoMatriculaC.Edt_AnoKeyPress(Sender: TObject;
          var Key: Char);
begin
     if not (Key in ['0'..'9',#3, #8, #22, #24]) then Key := #0;
end;

procedure TFrm_SelecionaAlunoMatriculaC.Button2Click(Sender: TObject);
begin
     close;
end;

procedure TFrm_SelecionaAlunoMatriculaC.Edt_AnoExit(Sender: TObject);
begin
     if not aFuncoes.ValidaAno(Edt_Ano.Text) THEN
        Edt_Ano.SETFOCUS;
end;

procedure TFrm_SelecionaAlunoMatriculaC.Edt_serieExit(Sender: TObject);
begin
...
procedure TFrm_SelecionaAlunoMatriculaC.Spb_LocalizaSerieClick(Sender: TObject);
begin
     LocalizaSerie('');
end;

//------------------------------------------------------------------------------
procedure TFrm_SelecionaAlunoMatriculaC.LocalizaSerie(WPPrimLetra: string);
//------------------------------------------------------------------------------
begin
     Edt_serie.Setfocus;
