unit U_EditaMensalidade;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, StdCtrls, Buttons, ExtCtrls, Mask, FnpNumericEdit, DB;

type
  TFrm_EditaMensalidade = class(TForm)
    Panel1: TPanel;
    Panel3: TPanel;
    Label1: TLabel;
    Label3: TLabel;
...
var
  Frm_EditaMensalidade: TFrm_EditaMensalidade;
...
procedure TFrm_EditaMensalidade.edt_mens_mesKeyPress(Sender: TObject; var Key:
          Char);
begin
     if not (Key in ['0'..'9',#3, #8, #22, #24]) then Key := #0;
end;

procedure TFrm_EditaMensalidade.AtribuiVariavel(WPQuery : TDataSet);
begin
     with WPQuery do
     begin
...
procedure TFrm_EditaMensalidade.LimpaVariavel(WPAno, WPMes, WPAluno : string);
begin
     ...
     mke_mensdtvc.text     := '';
     mke_mensdtpg.text     := '';
     chk_menspago.checked  := false;
end;


procedure TFrm_EditaMensalidade.FormShow(Sender: TObject);
begin
...
procedure TFrm_EditaMensalidade.FormKeyPress(Sender: TObject; var Key: Char);
begin
     If (Key = #13) then
     begin
        key:= #0;
...
procedure TFrm_EditaMensalidade.FormClose(Sender: TObject; var Action:
          TCloseAction);
begin
...
procedure TFrm_EditaMensalidade.Mke_mensdtvcExit(Sender: TObject);
begin
     if not aFuncoes.ValidaData( (Sender as TMaskEdit).text ) then
        (Sender as TMaskEdit).setfocus;
end;

procedure TFrm_EditaMensalidade.edt_mens_anoExit(Sender: TObject);
begin
...
procedure TFrm_EditaMensalidade.edt_mens_mesExit(Sender: TObject);
begin
...
procedure TFrm_EditaMensalidade.edt_mensalunExit(Sender: TObject);
begin
...
procedure TFrm_EditaMensalidade.Spb_localizaalunoClick(Sender: TObject);
begin
     LocalizaAluno('');
end;

//------------------------------------------------------------------------------
procedure TFrm_EditaMensalidade.LocalizaAluno(WPPrimLetra: string);
//------------------------------------------------------------------------------
begin
     Edt_mensalun.Setfocus;
...
procedure TFrm_EditaMensalidade.RetornaSaidas;
begin
...
procedure TFrm_EditaMensalidade.Button1Click(Sender: TObject);
begin
...
//------------------------------------------------------------------------------
procedure TFrm_EditaMensalidade.GravaRegistro(WPTipo: integer);
//------------------------------------------------------------------------------
begin
...
//------------------------------------------------------------------------------
function TFrm_EditaMensalidade.TestaCampoVazio: Boolean;
//------------------------------------------------------------------------------
begin
     Result:=False;
...
procedure TFrm_EditaMensalidade.Button4Click(Sender: TObject);
begin
